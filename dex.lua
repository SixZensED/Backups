		...
		local function main()
			...
			Explorer.ShowRightClick = function()
				...
				if presentClasses["BasePart"] or presentClasses["Model"] then
					...
					context:AddRegistered("TWEEN_TO")
			...
			Explorer.InitRightClick = function()
				...
				context:Register("TWEEN_TO",{Name = "Tween To", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
					...
					for i = 1,#sList do
						local node = sList[i]
						if isa(node.Obj,"BasePart") then
							totarget({CFrame = node.Obj.CFrame + Settings.Explorer.TeleportToOffset})
							break
						elseif isa(node.Obj,"Model") then
							totarget({CFrame = node.Obj:GetPivot()})
						end
					end
				end})
				...
				context:Register("SAVE_INST",{Name = "Save to File", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
					saveplace({FileName = tostring(game.PlaceId), IgnoreArchivable = true})
				end})
...
Main = (function()
	...
	Main.Init = function()
		...
		intro.SetProgress("Fetching Roblox Version",0)
		if Main.Elevated then
		...
		-- Fetch external deps
		intro.SetProgress("Fetching API",0)
		API = Main.FetchAPI()
		Lib.FastWait()
		intro.SetProgress("Fetching RMD",0)
		RMD = Main.FetchRMD()
		Lib.FastWait()
		...
		-- Load other modules
		intro.SetProgress("Loading Modules",0)
		Main.AppControls.Lib.InitDeps(Main.GetInitDeps()) -- Missing deps now available
		Main.LoadModules()
		Lib.FastWait()

		-- Init other modules
		intro.SetProgress("Initializing Modules",0)
		Explorer.Init()
		Properties.Init()
		ScriptViewer.Init()
		Lib.FastWait()
